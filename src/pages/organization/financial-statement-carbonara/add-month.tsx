...
import { Month } from '~/zod-schemas/monthly-fs';
...
type CreateMonthInputs = z.infer<typeof schemas.shared.monthlyFS.create>;
...
export default function CBLPage() {
  ...
  const createMonthForm = useForm<CreateMonthInputs>({
    resolver: zodResolver(schemas.shared.monthlyFS.create),
  });

  const createMonth = api.shared.monthlyFS.create.useMutation({
    ...
    onSuccess: async ({ id }) => {
      ...
      await router.push(`${paths.ORGANIZATION}${paths.FINANCIAL_STATEMENT}`);
    },
