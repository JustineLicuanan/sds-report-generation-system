...
type EditFSInflowRowInputs = z.infer<typeof schemas.shared.FSInflowRow.updateCollection>;

export default function EditCollectionRowPage() {
  const router = useRouter();
  const { monthlyID, FSInflowID, inflowRowId } = router.query;
  const utils = api.useContext();
  const { toast } = useToast();

  const getFSInflowRowQuery = api.shared.FSInflowRow.get.useQuery({
  ...
  const createFSInflowRowForm = useForm<EditFSInflowRowInputs>({
    resolver: zodResolver(schemas.shared.FSInflowRow.updateCollection),
  ...
  const createFSInflowRow = api.shared.FSInflowRow.updateCollection.useMutation({
    ...
    onSuccess: async () => {
      ...
      router.push(
        `${paths.ORGANIZATION}${paths.ORGANIZATION_REPORTS}${paths.FINANCIAL_STATEMENT}/${monthlyID}${paths.MODIFY_FINANCIAL_STATEMENT}/${paths.INFLOWS}/${FSInflowID}${paths.ADD_INFLOW}`
      );
    },
        ...
        >
          ...
          <div className="flex justify-end gap-4">
            <button
              ...
              onClick={() =>
                router.push({
                  pathname: `${paths.ORGANIZATION}${paths.ORGANIZATION_REPORTS}${paths.FINANCIAL_STATEMENT}/${monthlyID}${paths.MODIFY_FINANCIAL_STATEMENT}${paths.INFLOWS}/${FSInflowID}${paths.ADD_INFLOW}`,
                  query: { monthlyID: monthlyID, FSInflowID: FSInflowID },
                })
              }
