...
export default function CommunityExtensionServicesPage() {
  ...
  const createARGeneratedForm = useForm<CreateARGeneratedInputs>({
    ...
    defaultValues: {
      ...
      contentType: ARGeneratedContentType.COMMUNITY_EXTENSION_SERVICES,
      contentNumber: 1,
  ...
  const createARGenerated = api.shared.ARGenerated.create.useMutation({
    onSuccess: async ({ id }) => {
      ...
      await router.push({
        pathname: `${paths.ORGANIZATION}${paths.ORGANIZATION_REPORTS}${paths.ACCOMPLISHMENT_REPORT}${paths.GENERATED_FILES}${paths.COMMUNITY_EXTENSION_SERVICES}`,
